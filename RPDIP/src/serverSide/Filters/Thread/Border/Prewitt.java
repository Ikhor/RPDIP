 ...
 * @author Akrios
 */
public class Prewitt extends Thread {
    ...
    int maskX[] = {-1, -1, -1, 0, 0, 0, 1, 1, 1};
    int maskY[] = {-1, 0, 1, -1, 0, 1, -1, 0, 1};

    public Prewitt(BufferedImage image, BufferedImage aux, int threadID, int numThread, int tamMatriz) {
        this.image = image;
        this.aux = aux;
    ...
    public void run() {
        ...
        for (int i = 0; i < imgWidthThread; i++) {
            for (int j = 0; j < image.getHeight(); j++) {
                int[] pixel = PrewittPrivate(i + offSet, j, image);
                Color color;
                if (n == 1) {
    ...
    int[] PrewittPrivate(int linha, int coluna, BufferedImage image) {
        try {
            int n = image.getRaster().getNumDataElements();
