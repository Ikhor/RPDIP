 ...
 * @author Akrios
 */
public class Laplaciano extends Thread {
    ...
    public Laplaciano(BufferedImage image, BufferedImage aux, int threadID, int numThread, int tamMatriz) {
        this.image = image;
        this.aux = aux;
    ...
    public void run() {
        ...
        for (int i = 0; i < imgWidthThread; i++) {
            for (int j = 0; j < image.getHeight(); j++) {
                int[] pixel = LaplacianoPrivate(i + offSet, j, image);
                Color color;
                if (n == 1) {
